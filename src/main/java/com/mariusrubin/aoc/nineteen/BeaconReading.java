...
class BeaconReading {
  ...
  BeaconReading(final BeaconReading beaconReading) {
    x = beaconReading.getX();
    y = beaconReading.getY();
    z = beaconReading.getZ();
  }

  BeaconReading(final int x, final int y, final int z) {
    this.x = x;
    this.y = y;
    this.z = z;
  }

  int distanceTo(final BeaconReading other) {
    return Math.abs(x - other.x) + Math.abs(y - other.y) + Math.abs(z - other.z);
  }

  BeaconReading vectorise(final BeaconReading other) {
